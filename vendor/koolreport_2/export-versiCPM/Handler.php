...
class Handler
{
    ...
    public function settings($settings)
    {
        ...
        $this->autoDeleteLocalTempFile = Util::get($settings, "autoDeleteLocalTempFile");
        return $this;
    }
    ...
    protected function runPhantom($script, $source, $output, $params)
    {
        ...
        //if not asnb di comment,
        //$script = '/var/www/html/staging/asnb/web/CPM/vendor/koolreport/export/pdf/pdf.js';
        
	$export_server = strtolower(env('EXPORT_SERVER'));
        switch ($export_server)
        {
            case 'linux' :
            case 'docker_linux' :
                $dir_linux = env('EXPORT_DIR') . '/vendor/koolreport/export/';
		if($export_server == 'docker_linux')
		    $this->phantomjs = $dir_linux.'/bin/phantomjs';
                if (strpos($script, 'pdf/pdf.js') !== false)
                    $script = $dir_linux . 'pdf/pdf.js';
                elseif (strpos($script, 'image/image.js') !== false)
                    $script = $dir_linux . 'image/image.js';
                break;
        }
	...
	if (file_put_contents($source, $content)) {
        ...
        //tuk Debug (AS)
        /*
        $source = "C:/xampp2/htdocs/CPM/public/tmp/611b874f8c03a1.tmp";
    ...
    protected function getTempFolder()
    {
        if ($this->useLocalTempFolder) {
            ...
            // dd(realpath($path)."/tmp"); // "/var/www/html/CPM/public/tmp"
        ...
        // dd(sys_get_temp_dir()); //return /tmp
        return sys_get_temp_dir();
        //return '/var/www/html/CPM/storage/tmp'; //for test ASNB
    }
